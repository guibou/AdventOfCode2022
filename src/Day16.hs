...
solve' (makeASmallerGraph -> valves) = go (ValveId "AA", ValveId "AA") (Set.singleton (ValveId "AA")) (0, 0)
  where
    go currentValves visited currentCosts =
       ...
       in do
            if null motions
              then pure []
                ...
                ((nextValves, nextCosts) :) <$> go nextValves nextVisited nextCosts

nothingSecond ((a, b), v) = ((nt a, nt b), v)

nt (a, b) = (Just a, Nothing)
...
day' valves = Data.List.maximum $ map (weightPath' valves) $ solve' valves
-- * Tests
